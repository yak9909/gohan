    ...
    namespace GuiMenu
    {
        namespace
        {
            ...
            const char *Trim(const char *src, int maxWidth, char *buf, size_t cap,
                             GuiRenderer::Font font = GuiRenderer::FONT_MAIN)
            {
            ...
            //   不透明時も同じ絵になる。
            void    Frame1px(GuiRenderer::Screen sc, int x, int y, int w, int h,
                             u32 line, u32 fill)
            {
                if (w <= 2 || h <= 2)
                {
            ...
            // drawScrollBar の写し
            void    ScrollBar(GuiRenderer::Screen sc, int x, int y, int h,
            ...
            void    DrawDescription(float amount)
            {
                ...
                GuiRenderer::DrawText(TOP, x + 8, y + 7,
            ...
            void    DrawInlineList(int menuX, float start, u32 now)
            {
                ...
                while (i <= last)
                {
                    ...
                    {
                        if (i == g_inline.index)
                            GuiRenderer::FillRect(TOP, x + 3, rowY, 122, 12,
                                            Fade(kColListSel, amount));
                        GuiRenderer::DrawText(TOP, x + 8, rowY + 2,
            ...
            // 下: 呼ぶ側が dim を敷いてから呼ぶ。
            void    DrawScreenListbox(GuiRenderer::Screen sc, int baseX, int width,
                                      int offscreenX, u32 now)
            {
                ...
                const GuiRenderer::Font rowFont = g_overlay.mode == OV_CHAT_KANJI
                                            ? GuiRenderer::FONT_GAME : GuiRenderer::FONT_MAIN;
                ...
                Frame1px(sc, x, y, width, h, Fade(kColAccent, amount),
                         Fade(kColBoxBg, amount));
                GuiRenderer::DrawText(sc, x + 8, y + 7,
                ...
                while (i <= last)
                {
                    ...
                    if (rowY >= y + 20 && rowY + 8 <= y + h - 2)
                    {
                        if (i == g_overlay.index)
                            GuiRenderer::FillRect(sc, x + 5, rowY - 4, width - 14, 16,
                                            Fade(kColListSel, amount));
            ...
            void    DrawHotkeyCapture(void)
            {
                ...
                GuiRenderer::DrawText(BOT, x + 8, y + 8, "HOTKEY INPUT", kColAccent);
                GuiRenderer::DrawText(BOT, x + 8, y + 24, status, kColWhite);
                if (g_capture.captured != 0)
                    ...
                    GuiRenderer::DrawText(BOT, x + 8, y + 37, "--", kColHotkey);
                GuiRenderer::DrawText(BOT, x + 96, y + 37, u8"全て離すと決定", kColHint);
                bx = 0;
                while (bx < 2)
                {
                    ...
                    GuiRenderer::DrawText(BOT, kx + (100 - tw) / 2, y + 61, label,
                                    kColKeyTxt);
                    bx++;
                }
            }
            ...
            void    DrawNotices(u32 now)
            {
                ...
                while (i < kNoticeMax)
                {
                    ...
                    if (NoticeAlive(nt, now) && (s32)(now - nt.createdAt) >= 0)
                    {
                        ...
                        GuiRenderer::FillRect(TOP, x + 1, y + 1, 2, kNoticeH - 2,
                                        nt.accent);
                        GuiRenderer::DrawText(TOP, x + 7, y + 4,
                                        Trim(nt.title, kNoticeW - 20,
                                             g_buf, sizeof(g_buf)),
                                        kColWhite);
                        GuiRenderer::DrawText(TOP, x + 7, y + 14,
            ...
            void    BuildTop(u32 now)
            {
                ...
                GuiRenderer::DrawText(TOP, menuX + 91, 7,
                ...
                // ---- 選択帯 ----
                {
                    ...
                    GuiRenderer::FillRect(TOP, hx, hy - 3, kMenuW - 10, 15,
                ...
                // ---- 行（★切り抜きが無いので窓に入る行だけ描く）----
                {
                    ...
                    while (i <= last)
                    {
                        ...
                        GuiRenderer::DrawText(TOP, menuX + 8 + off, y, ItemIcon(it),
                                        it.type == ITEM_FOLDER ? kColAccent : col);

                        FormatValue(it, g_buf2, sizeof(g_buf2));

                        const GuiRenderer::Font vf = IsNumericItem(it) ? FNUM
                                                                 : GuiRenderer::FONT_MAIN;
                        const int   vw = g_buf2[0] != '\0'
                                       ? GuiRenderer::MeasureText(g_buf2, 1, vf) : 0;

                        GuiRenderer::DrawText(TOP, menuX + 27 + off, y,
                                        Trim(it.label, kMenuW - 33 - vw,
                                             g_buf, sizeof(g_buf)), col);
                        if (g_buf2[0] != '\0')
                            GuiRenderer::DrawText(TOP, menuX + kMenuW - 8 - vw + off, y,
                                            g_buf2, col, 1, vf);
                        if (it.type != ITEM_FOLDER && it.hotkey != 0)
                            GuiRenderer::DrawText(TOP, menuX + 147 + off, y + 8,
                                            "H", kColHotkey);
                        i++;
                    }
                }
                ...
                GuiRenderer::DrawText(TOP, menuX + 6, 230, g_buf,
            ...
            void    BuildBottom(u32 now)
            {
                ...
                // ★暗幕は GuiRenderer の「下画面ロック」が出す（F-350）。
        ...
        void    SyncInputLock(void)
        {
            ...
            GuiRenderer::SetButtonBlock(g_visible || (g_overlay.active && g_overlay.mode == OV_CHAT_KANJI));
            ...
            if (keyboard)
                GuiRenderer::SetBottomLock(true, GuiKeyboard::DimColor(),
                                     GuiKeyboard::DimFadeMs());
            else if (listbox || capture)
                GuiRenderer::SetBottomLock(true, kColDim, kListAnimMs);
            else
