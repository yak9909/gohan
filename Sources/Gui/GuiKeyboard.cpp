...
namespace {
    ...
    void Rect(int x,int y,int w,int h,uint32_t c,float a) { GuiRenderer::FillRect(bot,x,y,w,h,Fade(c,a)); }
    void Label(int x,int y,const char *t,uint32_t c,float a,bool numeric=false) {
        GuiRenderer::DrawText(bot,x,y,t,Fade(c,a),1,numeric?GuiRenderer::FONT_NUM:GuiRenderer::FONT_MAIN);
    }
    ...
    void DrawKey(const KeyRect &r,float a) {
        ...
        int width=GuiRenderer::MeasureText(label,1,numeric?GuiRenderer::FONT_NUM:GuiRenderer::FONT_MAIN);
    ...
    if(s.kind==NUMBER) {
        ...
        if(tap && tx>=inputX && tx<inputX+(s.compact?242:274) && ty>=inputY && ty<inputY+(s.compact?21:25)) {
            ...
            for(int i=0;i<=s.length;++i) {
                ...
                int d=std::abs(tx-((s.compact?45:31)+GuiRenderer::MeasureText(prefix)));
...
void Draw(uint32_t now) {
    ...
    if(s.kind==NUMBER) {
        ...
        if((now/500)%2==0) {
            ...
            Rect((s.compact?45:31)+GuiRenderer::MeasureText(prefix),s.compact?42:32,1,s.compact?11:12,0xFFA4E463,a);
        }
