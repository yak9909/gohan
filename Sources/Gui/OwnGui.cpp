...
//   v2: ケーブ 3 本（A1/A2/N）      + フック 2 本 + nw::lyt ヒープから GuiRenderer::BorrowBytes()
//       描画はゲームの Layout_RecordPaneTree がやるので記録ケーブが要らない。
//
// ★ケーブは Includes/GuiCaves.h のバイト列をそのまま書く。C++ へ書き直さない。
...
namespace CTRPluginFramework
{
    namespace
    {
                ...
                while (i < cv.count)
                {
                    ...
                    if (std::strcmp(cv.name, "N") == 0)
                    {
                        if (i == kGuiCaveNTexVaIndex)
                            v = GuiRenderer::AtlasVa();
                        else if (i == kGuiCaveNTexLenIndex)
                            v = GuiRenderer::AtlasBytes();
                    }
        ...
        bool    Enable(void)
        {
            ...
            // 2. .bss を初期化する
            //    ★★★順序が命（F-311 で実際に踏んだ）。
            //      初版はここが GuiRenderer::Install() の**あと**にあった。
            ...
            Log("アトラス 0x%08X (%u B)", (unsigned int)GuiRenderer::AtlasVa(),
                (unsigned int)GuiRenderer::AtlasBytes());
