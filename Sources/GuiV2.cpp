...
//
// ★数値の正本
//   ・ケーブと番地      … Includes/GuiCavesV2.h（PATCHES/export_gui_v2.py が生成）
...
namespace CTRPluginFramework
{
    namespace GuiV2
    {
        namespace
        {
            ...
            const int   kMaxRectTop  = 74;      // 上画面の矩形（F-343 で 56 -> 86、F-344 で 74）
            const int   kMaxRectBot  = 336;      // 下画面の矩形（据え置き）
            const int   kMaxRect     = 336;      // ★配列の大きさ。上下の大きい方
            const int   kMaxRectAll  = 410;     // Picture の総数（上 + 下）
            ...
            //   内訳（F-325。段 6/段 7 で追加。F-344 で通知は 5 件。合計 上 69 本 / 下 11 本）:
            //     上  2 字 x 4  … ホットキー印 H
            //         4 字 x10  … 行のアイコン（"[X]" が最長）
            //         8 字 x10  … 行の値（"OFF" / "0x2001" / リストの選択肢）
            //         8 字 x 9  … 上画面リストボックス（見出し + 8 行。選択肢は短い）
            //        16 字 x 8  … 見出し 2 + インラインリスト・ダイアログ 6
            //        16 字 x10  … 行の項目名（Trim で約 97px 以内に切るので入る）
            //        16 字 x 3  … 下帯 3 行
            //        16 字 x 5  … 通知の本文（項目名だけなので入る）★F-344 で 5
            //        24 字 x 5  … 通知の題（"CHEAT ENABLED 999" = 17 字）★F-344 で 5
            //        28 字 x 5  … 説明パネル（題 + HOTKEY + 本文 3 行）
            //     下  8 字 x 9  … 下画面リストボックス（見出し + 8 行）か
            //                     入力待ちの短文 4 のどちらか（単一 overlay なので共用）
            //        16 字 x 1  … 入力待ちの見出し（"HOTKEY INPUT" = 12 字）
            //        28 字 x 1  … 入力待ちの組み合わせ表示
            //   ★項目名・下帯は 24 字枠から 16 字枠へ移した。
            ...
            const u8    kTopSlotCaps[] = {
                ...
                16, 16, 16,
                16, 16, 16, 16, 16,             // ★通知の本文（F-344 で 5 件）
                24, 24, 24, 24, 24,             // ★通知の題（F-344 で 5 件）
                28, 28, 28, 28, 28
            };
            ...
            //   ★この値は OwnGui::Enable() が GuiV2::BorrowBytes() で読む。
            ...
            //   ほかは全部 GuiV2 の g_mem（プラグインの .bss）に置く。
        ...
        bool    Install(void)
        {
            ...
            Log("GuiV2 組み立て完了（矩形 %d 枚 / 文字スロット 上 %d・下 %d）",
