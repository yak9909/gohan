...
//   - 空き枠 = 生の中身が空（Item_IsEmpty 0x2FCB24）の枠。★ゲームの空き判定（Inventory_FindEmptySlot 0x723604）は「持てない物が入った枠」も
//     空とみなすので使わない（没アイテムを上書きしてしまう）。
...
namespace CTRPluginFramework
{
    namespace Cheats
    {
        namespace
        {
            ...
            // 没アイテム表示（連動型リスト: 0 非表示 / 1 表示 / 2 自前表示（未設計））
            bool    HiddenRead(int index, s32 *value)
            {
                (void)index;
                *value = PocketItem::HiddenShown() ? 1 : 0;
                return true;
            }
            ...
            void    HiddenWrite(int index, s32 value)
            {
                ...
                if (value == 2)
                {
                    GuiNotification::NotifyRed(kHiddenItems, u8"自前表示は未設計です（何もしません）");
                    return;
                }

                const PocketItem::Result r = PocketItem::SetHiddenShown(value == 1);
