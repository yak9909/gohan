...
// カーソルのマス: ゲームの UnitCursor をマス指定で並べる（GridCursor::ShowTiles）。形は**衝突判定**
//   （プレイヤーが歩けないマス）: 足元データ `Strc/data/<名>.bin` の属性コードをゲームの分類表
//   `byte_957B34`（sub_5CD3C0）で引いて 0 でないマス（IDA-opus-5.5-F017）。高さは全部そろえて設置プレビューと同じ。
//   色: 配置 = そのまま、移動 = 青、削除 = 赤。
...
//   L / R         … モード（配置 → 移動 → 削除）
//   十字 左右      … 配置する建物を替える（配置モード。モデルの読み込みは押す手が止まってから）
